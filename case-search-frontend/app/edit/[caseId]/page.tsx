...
export default function EditCase() {
        ...
        setFormData({
          ...
          accused: fetchedCaseData.accused && Array.isArray(fetchedCaseData.accused)
            ? fetchedCaseData.accused.map((acc: any) => ({
              ...
              chargesheet: {
                date: acc.chargesheet?.date ? new Date(acc.chargesheet.date).toISOString().split('T')[0] : "",
                file: acc.chargesheet?.file || null,
              },
            }))
            : [],
        });
  ...
  const updateAccusedChargesheet = (index: number, field: string, value: any) => {
    setFormData(prev => ({
      ...prev,
      accused: prev.accused.map((acc, i) => {
        if (i === index) {
  ...
  const uploadFile = async (file: File): Promise<{ public_id: string; secure_url: string; url: string; original_filename: string; format: string; bytes: number } | null> => {
    try {
      return await handleFileUpload(file, 'case-reports/accused-chargesheet');
    } catch (error: any) {
                ...
                <div className="space-y-4">
                  {formData.accused.map((accused, index) => (
                    <div key={index} className="border border-slate-200 rounded-lg p-4 bg-white">
                      ...
                      {/* Chargesheet Section */}
                      <div className="mt-4 pt-4 border-t border-slate-200">
                        <h5 className="text-xs font-semibold text-slate-700 mb-3">Chargesheet</h5>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-xs font-medium text-slate-600 mb-1">Chargesheet Date</label>
                            <input
                              type="date"
                              value={accused.chargesheet?.date || ""}
                              onChange={(e) => updateAccusedChargesheet(index, "date", e.target.value)}
                              className="w-full px-2 py-1.5 rounded border border-slate-300 bg-white focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-xs"
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-slate-600 mb-1">Chargesheet File (PDF/Image)</label>
                            {accused.chargesheet?.file ? (
                              <div className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded">
                                <span className="text-xs text-slate-700 truncate max-w-[150px]">{accused.chargesheet.file.original_filename}</span>
                                <button
                                  type="button"
                                  onClick={async () => {
                                    if (accused.chargesheet?.file?.public_id) {
                                      await deleteFile(accused.chargesheet.file.public_id);
                                    }
                                    updateAccusedChargesheet(index, "file", null);
                                  }}
                                  className="text-xs text-red-600 hover:text-red-800"
                                >
                                  Remove
                                ...
                                onChange={(e) => {
                                  ...
                                  if (file) {
                                    uploadFile(file).then((uploadedFile) => {
                                      if (uploadedFile) {
                                        updateAccusedChargesheet(index, "file", uploadedFile);
                                      }
                                    });
                                  }
                                }}
